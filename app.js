const trip = {
  ...
  days: [
    { date:'Qui · 27 ago', city:'Palma', title:'Chegada à ilha', image:'./assets/palma.jpg', items:[
      ...
      {time:'22:30',title:'Chegada, check-in e mercado',description:'Ir à hospedagem em Palma, acomodar o grupo e comprar o básico no mercado.',category:'hospedagem',image:'./assets/palma.jpg',maps:'Carrer de la Mar 15, Palma de Mallorca',cost:'Mercado estimado: €15–25'},
    ]},
    { date:'Sex · 28 ago', city:'Costa sul', title:'Calas e AMØK', image:'./assets/calo-des-moro.jpg', items:[
      {time:'07:00',title:'Saída para as calas',description:'Saída antecipada para conseguir aproveitar Caló des Moro antes do pico de lotação.',category:'transporte',image:'./assets/calo-des-moro.jpg',maps:'Caló des Moro, Mallorca',cost:'Transporte estimado: €20–35'},
      ...
      {time:'23:45',title:'AMØK Mallorca',description:'Entrada confirmada às 23h45. Chegue com antecedência para não perder a janela planejada.',category:'festa',image:'./assets/boat-night.jpg',maps:'AMOK Mallorca',cost:'Ingresso: €15,00 já pago'},
    ]},
    { date:'Sáb · 29 ago', city:'Palma e Magaluf', title:'Praia, aniversário e BCM/Titos', image:'./assets/cala-llombards.jpg', items:[
      ...
      {time:'20:00',title:'Jantar de aniversário',description:'Reserva feita. O depósito será convertido em consumo no local.',category:'refeição',image:'./assets/palma.jpg',maps:'Palma de Mallorca',cost:'Depósito: €10,00 já pago'},
      {time:'23:00',title:'BCM Planet Dance',description:'Saída para a balada. A entrada deve ocorrer antes da meia-noite.',category:'festa',image:'./assets/boat-night.jpg',maps:'BCM Planet Dance Magaluf',cost:'Ingresso: €35,00 já pago'},
    ]},
    { date:'Dom · 30 ago', city:'Palma', title:'Centro, barco e jantar', image:'./assets/palma.jpg', items:[
      ...
      {time:'20:00',title:'Jantar de despedida',description:'Jantar em Palma após o barco e organização para a saída de madrugada.',category:'refeição',image:'./assets/palma.jpg',maps:'Palma de Mallorca',cost:'Táxi estimado: €10–15, além do jantar'},
    ]},
  ...
  costs: [
    ...
    {id:'amok',category:'Experiências',title:'AMØK Mallorca',note:'Ingresso, entrada às 23h45',amount:15},
    {id:'club',category:'Experiências',title:'BCM Planet Dance',note:'Ingresso, entrada antes de 00h',amount:35},
...
const forecast=[
  {title:'Mercado na chegada',range:'€15–25',note:'Quinta-feira, após check-in'},
  {title:'Transporte local',range:'€40–70',note:'Praias, Palma, Magaluf e táxi de domingo'},
  {title:'Alimentação',range:'€125–175',note:'Brunches, jantares e refeições do roteiro'},
  {title:'Bebidas',range:'€120–215',note:'Estimativa das noites e do barco'},
  {title:'Praia e extras',range:'€40–75',note:'Provisão para gastos de praia não detalhados'},
];
...
function renderCosts(){const list=trip.costs.filter(x=>state.filter==='all'||x.category===state.filter);$('#plannedTotal').textContent=fmt(trip.paidTotal);$('#home-budget').textContent=`${fmt(trip.paidTotal)} já pago`;const paid=paidAmount();$('#paidSummary').textContent=`${fmt(paid)} marcado como conferido`;$('#paidPercent').textContent=`${Math.round(paid/trip.paidTotal*100)}%`;$('#costList').innerHTML=list.map(x=>`<button class="cost-item ${state.paid.includes(x.id)?'paid':''}" data-cost="${x.id}"><span class="cost-check">${state.paid.includes(x.id)?'✓':''}</span><span><small>${x.category}</small><b>${x.title}</b><em>${x.note}</em></span><strong>${fmt(x.amount)}</strong></button>`).join('');$('#forecastList').innerHTML=forecast.map(x=>`<div class="forecast-row"><span><b>${x.title}</b><small>${x.note}</small></span><strong>${x.range}</strong></div>`).join('');$('#expenseList').innerHTML=state.expenses.length?state.expenses.map((x,i)=>`<div class="expense-row"><span><b>${x.label}</b><small>${x.category}</small></span><strong>${fmt(x.amount)}</strong><button data-expense="${i}" aria-label="Excluir gasto">×</button></div>`).join(''):'<p class="empty">Nenhum gasto real registrado ainda.</p>'; }
