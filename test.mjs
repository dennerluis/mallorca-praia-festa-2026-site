...
assert.match(html,/Mallorca Praia & Festa 2026/);assert.equal(manifest.display,'standalone');assert.match(app,/Magic Catamarans/);assert.match(app,/Google Maps/);assert.match(app,/paidTotal: 414/);assert.match(app,/AMØK Mallorca/);assert.match(app,/BCM Planet Dance/);assert.match(app,/31 ago/);assert.match(app,/register\('\.\/sw\.js'\)/);assert.match(storage,/indexedDB/);assert.match(storage,/navigator\.storage/);assert.doesNotMatch(html,/src="\/assets\//);console.log('Mallorca static site smoke tests passed');
